...
local function getCubeChest(player)
	return cubeChest
end

local function getRecipeBook(player)
	return theBook
end
...
local function getCubeContents(player)
	return types.Container.inventory(getCubeChest(player)):getAll()
end

--update to get the cube specefic to the player, when multiplayer becomes a thing
local function getCube(player)
	return theCube
end
...
local function e_rx_hadCube(data)
	...
	cubeChest = world.createObject("dwemer_cube", 1)
	theCube = world.createObject("misc_dwrv_ark_cube00", 1)
...
local function onSave()
	for _x, player in ipairs(world.players) do
